...
class TextGenerator(ABC):
    ...
    def __subclasscheck__(cls: ABCMeta, subclass: type) -> bool:
        return hasattr(subclass, "predict") and callable(subclass.predict)
...
class Chatbot(TextGenerator):
    ...
    def __init__(self, api_key: str = None, temperature: float = 0.7):
        ...
        self.__chatbot = ChatOpenAI(
            openai_api_key=self.__api_key, temperature=temperature
        )
...
def get_default_text_generator(
    temperature: float = 0.7, is_llm: bool = True
) -> TextGenerator:

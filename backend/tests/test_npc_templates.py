...
from src.npc_generation import (
    ...
    get_appearance_template,
    get_npc_relation_template,
    get_psychology_template,
)
...
def test_alignment_no_info_no_alignment_list_generate_for_all_alignments(
    ALL_ALIGNMENTS,
):
    # Arrange
    info = None
...
def test_alignment_template():
    info = (
        "A human that is a shopkeeper. That is a good person. The incarnation of good."
    )
    good_alignment = [
    ...
    assert (
        alignment_template
        == f"What is the alignment of the NPC? Based on this info: {info}. It must be one of the following: {good_alignment}, do not give any other answer."
    )
...
def test_get_psychology_template():
    profile = NPCProfile(
        "John", 25, Race.HUMAN, Alignment.LAWFUL_GOOD, True, "He is a hero."
    )

...
class RAG:
    ...
    def get_most_relevant_docs(self, query: str, k: int = 5, threshold: float = 0.7):
        ...
        relevant_indices = [
            idx for idx in sorted_indices if similarities[idx] >= threshold
        ]
    ...
    def relevant_docs_parent_retriever(
        self,
        query: str,
        ...
        return self.convert_docs_to_strings(
            parent_document_retriver.invoke(input=query)
        )

    def relevant_docs_ensemble_retrivers(
        self, query: str, ensemble_retriver: EnsembleRetriever
    ):
